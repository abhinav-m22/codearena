...
import { AiFillDislike, AiFillLike, AiFillStar, AiOutlineLoading3Quarters, AiOutlineStar } from 'react-icons/ai';
...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem }) => {
	...
	const handleLike = async () => {
		if (!user) {
			toast.error('You need to be logged in to like a problem', { position: 'top-left', theme: 'dark' });
			return;
		}
		...
		await runTransaction(firestore, async (transaction) => {
			...
			const { problemDoc, userDoc, userRef, problemRef } = await returnUserDataAndProblem(transaction);
	...
	const handleDislike = async () => {
		if (!user) {
			toast.error('You need to be logged in to dislike a problem', { position: 'top-left', theme: 'dark' });
			return;
		}
	...
	const handleStar = async () => {
		if (!user) {
			toast.error('You need to be logged in to star a problem', { position: 'top-left', theme: 'dark' });
			return;
		}
